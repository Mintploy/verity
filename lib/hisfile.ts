...
import {
  decryptFields, encryptFields, encryptValue, generateDataKey, isCiphertext, unwrapDataKey, wrapDataKey,
} from './crypto';
...
export interface HisFile {
  ...
  phone?: string;
  /**
   * Generated by Postgres from `phone` until his_files_drop_phone_normalized.sql
   * runs. Read-only, never send it back.
   */
...
/**
 * Ten digits, or null. Mirrors what the phone_normalized column computed and
 * what the phone HMAC is taken over. Keep in step with the audit log's
 * normalizePhoneDigits: the two must agree on what "the same number" means.
 */
...
/** True for PostgREST's "column does not exist" (after the old column is dropped). */
 ...
 *
 * Looks up by HMAC first. Rows written before the backfill have no HMACs, so
 * a miss falls through to the plaintext columns, which exist until
 * his_files_drop_phone_normalized.sql runs and are still readable while the
 * identity fields are plaintext. Once the column is gone or the fields are
 * ciphertext those fallbacks simply find nothing, which is correct: by then
 * every row has its HMACs.
 *
...
async function findDuplicateRaw(sb: SupabaseClient, userId: string, file: HisFile): Promise<HisFile | null> {
  ...
  if (digits) {
    const byHmac = await sb.from('his_files')
      .select('*').eq('user_id', userId).eq('phone_hmac', h.phone_hmac!)
      .order('researched_at', { ascending: true }).limit(1);
    if (byHmac.error) throw byHmac.error;
    if (byHmac.data?.[0]) return byHmac.data[0];

    const legacy = await sb.from('his_files')
      .select('*').eq('user_id', userId).eq('phone_normalized', digits)
      .order('researched_at', { ascending: true }).limit(1);
    if (legacy.error && !isMissingColumn(legacy.error)) throw legacy.error;
  ...
  // Legacy: rows with no HMACs and plaintext names. Filtered here rather than
  // in the query: names contain commas and quotes, which are PostgREST filter
  // syntax. A personal vault is small enough.
  const names = [normalizeName(file.full_name), normalizeName(file.nickname)].filter(Boolean);
  const { data } = await sb.from('his_files')
    .select('*').eq('user_id', userId).is('phone_hmac', null).is('name_hmac', null)
    .order('researched_at', { ascending: true });
  return (data ?? []).find((row: HisFile) =>
    (!isCiphertext(row.full_name) && names.includes(normalizeName(row.full_name)))
    || (!isCiphertext(row.nickname) && names.includes(normalizeName(row.nickname)))
  ) ?? null;
}
