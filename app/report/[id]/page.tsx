...
function ReportSidebar({ report, onCompare }: { report: Report; onCompare: () => void }) {
  const sections = ['Safety score', 'Phone intelligence', 'Identity', 'Address history', 'Property intelligence', 'Relationships', 'Professional', 'Public records', 'Social footprint', 'Recommendations'];
  return (
    <aside className="v-hide-mobile" style={{ position: 'sticky', top: 92, height: 'fit-content', display: 'flex', flexDirection: 'column', gap: 20 }}>
      <button onClick={() => history.back()} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px', border: 0, background: 'transparent', cursor: 'pointer', alignSelf: 'flex-start', fontFamily: 'var(--sans)', fontSize: 13, color: 'var(--dark-soft)' }}>
        ← New search
      </button>
      <div style={{ padding: 18, borderRadius: 'var(--r-lg)', background: 'var(--pearl)', boxShadow: 'var(--shadow-sm)' }}>
        ...
        {sections.map((s, i) => (
          <a key={i} href={`#sec-${i}`} style={{ fontFamily: 'var(--sans)', fontSize: 13, color: 'var(--dark-soft)', textDecoration: 'none', padding: '8px 10px', borderRadius: 8, display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <div id="sec-0" style={{ display: 'grid', gridTemplateColumns: '1.2fr 1.8fr', borderRadius: 'var(--r-xl)', overflow: 'hidden', boxShadow: `0 16px 50px ${scoreConfig.glow}, 0 2px 6px rgba(61,36,52,0.06)` }}>
        <div style={{ padding: '36px 36px 32px', background: scoreConfig.bg, position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1, height: '100%', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ fontFamily: 'var(--serif)', fontStyle: 'italic', fontSize: 18, color: scoreConfig.deep, opacity: 0.8, marginTop: 12 }}>{scoreConfig.sub}</div>
            <div style={{ flex: 1 }} />
          ...
          {report.addresses.map((a, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 14, padding: '12px 0', borderTop: i >= 2 ? '1px solid var(--gold-pale)' : 'none' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                style={{ fontFamily: 'var(--serif)', fontSize: 16, color: 'var(--dark)', lineHeight: 1.2, textDecoration: 'none', borderBottom: '1px solid var(--mauve)', display: 'inline-block' }}>{a.addr}{' ↗'}</a>
          ...
          <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
            {report.nextSteps.map((s, i) => (
              <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                <div style={{ fontFamily: 'var(--serif)', fontStyle: 'italic', fontSize: 16, color: 'var(--blush)', lineHeight: 1, flexShrink: 0, paddingTop: 2, width: 18, textAlign: 'right' as const }}>{String(i + 1).padStart(2, '0')}</div>
...
function Section({ id, eyebrow, title, children, accent = 'var(--blush-pale)' }: { id?: string; eyebrow: string; title: string; children: React.ReactNode; accent?: string }) {
  return (
...
function TagList({ items }: { items: string[] }) {
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
      {items.map((item, i) => (
        <span key={i} style={{ padding: '5px 12px', background: 'var(--ivory)', borderRadius: 'var(--r-pill)', fontFamily: 'var(--sans)', fontSize: 12.5, color: 'var(--dark)' }}>{item}</span>
      ))}
    </div>
  );
}
